...
For each page that has `testScenarios[]`, execute each scenario step via Playwright, screenshot after each step, run qa-vision.
...
Only file CLEAR visual issues. Check existing issues first to avoid duplicates.
...
## RULES
1. Screenshot EVERY state — mobile viewport 390x844
2. Wait for network idle
3. Batch bugs — test all pages first, then file issues
4. Check existing issues before filing
